...
import { defaultWeeklySchedule, formatScheduleSummary, getAdminRestaurant, isSupabaseConfigured, saveRestaurantToSupabase } from "@/lib/data/supabase-restaurant";
...
export default function CompanyPage() {
  ...
  function handleHeroImage(file?: File) {
    ...
    if (file.size > maxHeroImageSizeInBytes) {
      setMessage("A imagem do hero deve ter no máximo 5 MB.");
      return;
    }

    setHeroImageFile(file);
    setHeroPreviewUrl(URL.createObjectURL(file));
    setMessage("Nova imagem selecionada. Clique em Salvar configurações para publicar.");
  }

  function removeHeroImage() {
    setHeroImageFile(null);
    setHeroPreviewUrl(null);
    setForm((current) => ({ ...current, coverUrl: undefined }));
    setMessage("Imagem removida do hero. Clique em Salvar configurações para publicar.");
  }
  ...
  async function handleSave() {
    ...
    try {
      const coverUrl = heroImageFile ? await uploadHeroImage(heroImageFile) : form.coverUrl;
          ...
          <form className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-5 md:col-span-2">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">Banner exibido no topo do cardápio público. Recomendado: proporção 5:1, até 5 MB.</p>
              </div>

              <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white">
                {heroPreviewUrl || form.coverUrl ? (
                  <img
                    src={heroPreviewUrl ?? form.coverUrl}
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <label className="inline-flex h-9 cursor-pointer items-center justify-center gap-2 rounded-control border border-slate-200 bg-white px-3.5 text-sm font-medium text-slate-800 shadow-sm transition hover:bg-slate-50">
                  {heroPreviewUrl || form.coverUrl ? <Pencil className="h-4 w-4" /> : <ImagePlus className="h-4 w-4" />}
                  {heroPreviewUrl || form.coverUrl ? "Editar imagem" : "Adicionar imagem"}
                  <input
                    className="sr-only"
                    type="file"
                ...
                {(heroPreviewUrl || form.coverUrl) && (
                  <AdminButton variant="danger" onClick={removeHeroImage} type="button">
